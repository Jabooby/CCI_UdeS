...
uint16_t MS_Scan_Result[64];                     // Mouse Movement Scan Result
...
// usefull things
extern void vibrate(uint8_t value);
uint8_t Com_Transfer[64];
uint8_t Com_Transferred;
 ...
 * @return  none
 */
void TIM3_IRQHandlerDevice(void)
{
...
void USART2_Receive_Handle(void)
{
    ...
    // KB_Data_Pack[ 2 ] = DEF_KEY_CHAR_A;   //Added by RG to always send 'A' key
    // flag = 1;
    if (flag == 0)
    {
...
void MS_Scan(void)
{
        ...
        if (Com_Transferred)
    {
        ...
        //MS_Scan_Result[1] += 0x02; // sert à rien cos memset à 0

        // for( int i = 0; i < 4; i++ ){

        // MS_Data_Pack[i] = Com_Transfer[i + 2]; // fait un clic droit
        // }
    }
    // else if( scan_cnt >= 1 )
    // {
 ...
 *
 * datapack:    
 * [ 0 ]: 1 = left click, 2 = right click, 3 = ?
 * [ 1 ]: < 128 = right, > 128 = left
 * [ 2 ]: < 128 = down, > 128 = up
 */
void MS_Scan_Handle(void)
{
    ...
    if (MS_Scan_Done)
    {
        ...
        // if (joystick.btn) // for debug purposes
        //     MS_Data_Pack[2] += 254;
        flag = 1;
        for (i = 0; i < 4; i++)
        {
            MS_Data_Pack[i] = Com_Transfer[i];
        }

#if HAPTIC
        if (MS_Data_Pack[0]) // Haptic feedback-ish
            vibrate(1);
        else
            vibrate(0);
#endif
    ...
    if (flag)
    {
        ...
        memset(MS_Scan_Result, 0x00, DEF_ENDP_SIZE_MS);
        if (status == USB_SUCCESS)
        {
            /* Clear flag after successful loading */
            flag = 0;
