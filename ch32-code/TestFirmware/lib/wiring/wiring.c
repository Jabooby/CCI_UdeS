...
GPIO_TypeDef * CalcGPIOx (enum GPIOpins pin);
uint16_t CalcPinOffset (enum GPIOpins pin);
...
void pinMode(enum GPIOpins pin, enum GPIOpinMode mode) {
	GPIO_TypeDef * GPIOx;
	GPIOx = CalcGPIOx(pin);		//Verifier si cette ligne handle bien les pointeurs
	uint16_t PinOffset = (CalcPinOffset(pin)*4);
	
	if (PinOffset <= 7*4) GPIOx->CFGLR &= ~(0b1111<<PinOffset);	// zero the 4 configuration bits
	else GPIOx->CFGHR &= ~(0b1111<<(PinOffset-32));
	...
	// wirte mask to CFGR
	if (PinOffset <= 7*4)	GPIOx->CFGLR |= modeMask<<PinOffset;
	else GPIOx->CFGHR |= modeMask<<(PinOffset-32);
...
void digitalWrite(enum GPIOpins pin, uint8_t value) {
	...
	GPIO_TypeDef * GPIOx = CalcGPIOx(pin);
	uint16_t PinOffset = CalcPinOffset(pin);
